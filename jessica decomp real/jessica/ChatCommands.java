...
public class ChatCommands {
    ...
    public static void commands(final String message) {
        ...
        if (message.equalsIgnoreCase(".help")) {
            ...
            Wrapper.msg("&a.view [Nick] - See another player\u2019s camera in the render zone.", false);
            Wrapper.msg("&a.view off - for normal view.", false);
            Wrapper.msg("&a.nickchange [Nick] - Hide your nick.", false);
            Wrapper.msg("&a.getuuid - Get player's UUID.", false);
            Wrapper.msg("&a.getip - Get server ip.", false);
        ...
        if (message.equalsIgnoreCase(".creativefind")) {
            ...
            Wrapper.getFiles();
            txt = new File(String.valueOf(String.valueOf(FileManager.getClientDir())) + "CreativeFinder//" + Wrapper.mc().getCurrentServerData().serverIP.split(":")[0] + ".txt");
            if (((File)txt).exists()) {
                ((File)txt).delete();
            }
            for (NetworkPlayerInfo n : players) {
                ...
                Wrapper.getFiles();
                FileManager.write((File)txt, n.getGameProfile().getName());
            }
            Wrapper.msg("&eList of players in &6CREATIVE MODE &esaved in file", true);
            Wrapper.msg("&e" + ((File)txt).getPath(), false);
        }
        if (message.equalsIgnoreCase(".playerfind")) {
            connection = Wrapper.player().connection;
            players = GuiPlayerTabOverlay.ENTRY_ORDERING.sortedCopy(connection.getPlayerInfoMap());
            Wrapper.getFiles();
            txt = new File(String.valueOf(String.valueOf(FileManager.getClientDir())) + "PlayerFinder//" + Wrapper.mc().getCurrentServerData().serverIP.split(":")[0] + ".txt");
            if (((File)txt).exists()) {
                ((File)txt).delete();
            }
            for (NetworkPlayerInfo n : players) {
                ...
                public void run() {
                    try {
                        ...
                        for (NetworkPlayerInfo n : players) {
                            ...
                            GuiConnecting.networkManager2.sendPacket(new C00Handshake(String.valueOf(String.valueOf(s[1])) + "\u0000" + "32.123." + String.valueOf(rand.nextInt(255)) + "." + String.valueOf(rand.nextInt(255)) + "\u0000" + n.getGameProfile().getId().toString(), port, EnumConnectionState.LOGIN));
                            GuiConnecting.networkManager2.sendPacket(new CPacketLoginStart(new GameProfile(null, String.valueOf(String.valueOf(n.getGameProfile().getName())) + "B")));
                            Thread.sleep(0L);
                        }
                ...
                public void run() {
                    try {
                        ...
                        for (NetworkPlayerInfo n : players) {
                            ...
                            GuiConnecting.networkManager2.setNetHandler(new NetHandlerLoginClient(GuiConnecting.networkManager2, Minecraft.getMinecraft(), new GuiIngameMenu()));
                            GuiConnecting.networkManager2.sendPacket(new C00Handshake(String.valueOf(String.valueOf(IPKick)) + "\u0000" + "32.123." + String.valueOf(rand.nextInt(255)) + "." + String.valueOf(rand.nextInt(255)) + "\u0000" + n.getGameProfile().getId().toString(), PortKick, EnumConnectionState.LOGIN));
        ...
        if (message.equalsIgnoreCase(".getuuid")) {
            try {
                String uuid = Wrapper.player().getUniqueID().toString();
                Toolkit.getDefaultToolkit().getSystemClipboard().setContents(new StringSelection(uuid), null);
                Wrapper.msg(uuid, true);
                Wrapper.msg("&eCopied!", true);
            }
            catch (Exception e2) {
        ...
        if (message.equalsIgnoreCase(".getip")) {
            try {
                String ip = Wrapper.player().connection.getNetworkManager().getRemoteAddress().toString();
        ...
        if (args[0].equalsIgnoreCase(".bind")) {
            try {
                if (args[1].equalsIgnoreCase("add")) {
                    Wrapper.getBinds();
                    Binds.addBind(args[2], message.replace(String.valueOf(String.valueOf(args[0])) + " " + args[1] + " " + args[2] + " ", ""));
        ...
        if (args[0].equalsIgnoreCase(".view")) {
            try {
                if (args[1].equalsIgnoreCase("off")) {
                    Wrapper.mc().renderViewEntity = Wrapper.player();
                    Wrapper.msg("Normal view!", true);
                }
                for (Object o : Wrapper.world().loadedEntityList) {
                    EntityPlayer e;
                    if (!(o instanceof EntityPlayer) || !(e = (EntityPlayer)o).getName().equalsIgnoreCase(args[1])) continue;
                    Wrapper.mc().renderViewEntity = e;
                }
            }
            catch (Exception e4) {
                Wrapper.msg("&cError! Correct:", true);
                Wrapper.msg("&a.view [Nick]", true);
            }
        }
        ...
        if (message.startsWith(".getbaltop")) {
            new Thread(){
                ...
                public void run() {
                    try {
                        Wrapper.getFiles();
                        File txt = new File(String.valueOf(String.valueOf(FileManager.getClientDir())) + "BalTop\\" + Wrapper.mc().getCurrentServerData().serverIP.split(":")[0] + ".txt");
                        ...
                        String listNicks = "";
                        for (String s : list) {
                            listNicks = String.valueOf(String.valueOf(listNicks)) + s + "\r\n";
                        }
