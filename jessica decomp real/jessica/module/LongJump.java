...
extends Module {
    ...
    public LongJump() {
        super("LongJump", 0, Category.Player);
    }
    ...
    public void onMotion(MoveEvent event) {
        if (Wrapper.mc().gameSettings.keyBindJump.pressed) {
            ...
            if (LongJump.round(Wrapper.player().posY - (double)((int)Wrapper.player().posY), 3) == LongJump.round(0.138, 3)) {
                EntityPlayerSP player2;
                EntityPlayerSP player;
                EntityPlayerSP thePlayer = player = Wrapper.player();
                player.motionY -= 0.08;
                event.y -= 0.0931;
                EntityPlayerSP thePlayer2 = player2 = Wrapper.player();
                player2.posY -= 0.0931;
            }
