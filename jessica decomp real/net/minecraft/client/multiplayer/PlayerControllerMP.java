...
public class PlayerControllerMP {
    ...
    public boolean onPlayerDestroyBlock(BlockPos pos) {
        ...
        if (!this.currentGameType.isCreative() && !(itemstack2 = this.mc.player.getHeldItemMainhand()).func_190926_b()) {
    ...
    public boolean onPlayerDamageBlock(BlockPos posBlock, EnumFacing directionFacing) {
        ...
        Wrapper.onDamage(event);
        posBlock = event.pos;
        directionFacing = event.facing;
    ...
    public EnumActionResult processRightClickBlock(EntityPlayerSP player, WorldClient worldIn, BlockPos stack, EnumFacing pos, Vec3d facing, EnumHand vec) {
        ...
        float f2 = (float)(facing.yCoord - (double)stack.getY());
        float f3 = (float)(facing.zCoord - (double)stack.getZ());
        boolean flag = false;
        ...
        if (this.currentGameType.isCreative()) {
            ...
            EnumActionResult enumactionresult = itemstack.onItemUse(player, worldIn, stack, vec, pos, f, f2, f3);
    ...
    public void attackEntity(EntityPlayer playerIn, Entity targetEntity) {
        try {
            if (Wrapper.getModule("Reach").isToggled() && (double)Wrapper.player().getDistanceToEntity(targetEntity) > Reach.reach.getDoubleValue()) {
                return;
            }
            this.syncCurrentPlayItem();
