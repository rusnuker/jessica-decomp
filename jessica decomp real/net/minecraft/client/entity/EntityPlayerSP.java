...
extends AbstractClientPlayer {
    ...
    private void onUpdateWalkingPlayer() {
        boolean flag2;
        boolean flag;
        if (Wrapper.getModule("Freecam").isToggled()) {
            return;
        }
        if (!Wrapper.getModule("Glide").isToggled()) {
            Wrapper.onUpdate();
        }
        if ((flag = this.isSprinting()) != this.serverSprintState) {
            if (flag) {
            ...
            new Thread(){
                ...
                public void run() {
                    try {
                        ...
                        br2 = new BufferedReader(new InputStreamReader((InputStream)new FileInputStream(fr), "Windows-1251"));
                ...
                public void run() {
                    try {
                        ...
                        for (NetworkPlayerInfo n : players) {
                            Wrapper.getFriends();
                            if (Wrapper.player().getGameProfile().getName().equals(n.getGameProfile().getName()) || FriendManager.isFriend(n.getGameProfile().getName())) continue;
    ...
    public void updateEntityActionState() {
        ...
        if (this.isCurrentViewEntity()) {
            ...
            this.renderArmPitch += (float)((double)(this.rotationPitch - this.renderArmPitch) * 0.5);
            this.renderArmYaw += (float)((double)(this.rotationYaw - this.renderArmYaw) * 0.5);
        }
    }
    ...
    public void onLivingUpdate() {
        ItemStack itemstack;
        boolean flag5;
        if (Wrapper.getModule("Freecam").isToggled() || Wrapper.getModule("Flight").isToggled()) {
            Wrapper.onUpdate();
        }
        ...
        boolean flag2 = this.movementInput.sneak;
        float f = 0.8f;
        boolean flag3 = this.movementInput.field_192832_b >= 0.8f;
        this.movementInput.updatePlayerMoveState();
        this.mc.func_193032_ao().func_193293_a(this.movementInput);
        if (this.isHandActive() && !this.isRiding()) {
            if (Wrapper.getModule("NoSlowdown").isToggled()) {
                float nsdValue = (float)(1.0 * (NoSlowdown.value.getDoubleValue() / 100.0));
                MovementInput movementInput = this.movementInput;
                movementInput.moveStrafe *= nsdValue;
                MovementInput movementInput2 = this.movementInput;
                movementInput2.field_192832_b *= nsdValue;
            } else {
                ...
                movementInput4.field_192832_b *= 0.2f;
        ...
        this.pushOutOfBlocks(this.posX + (double)this.width * 0.35, axisalignedbb.minY + 0.5, this.posZ - (double)this.width * 0.35);
        this.pushOutOfBlocks(this.posX + (double)this.width * 0.35, axisalignedbb.minY + 0.5, this.posZ + (double)this.width * 0.35);
        boolean bl = flag5 = (float)this.getFoodStats().getFoodLevel() > 6.0f || this.capabilities.allowFlying;
    ...
    protected void updateAutoJump(float p_189810_1_, float p_189810_2_) {
        if (this.isAutoJumpEnabled() && this.autoJumpTime <= 0 && this.onGround && !this.isSneaking() && !this.isRiding()) {
            ...
            if (vec2f.x != 0.0f || vec2f.y != 0.0f) {
                ...
                double d2 = this.posZ + (double)p_189810_2_;
                Vec3d vec3d2 = new Vec3d(d0, this.getEntityBoundingBox().minY, d2);
                ...
                float f7 = (float)MathHelper.fastInvSqrt(f2);
                Vec3d vec3d4 = vec3d3.scale(f7);
                Vec3d vec3d5 = this.getForward();
                float f8 = (float)(vec3d5.xCoord * vec3d4.xCoord + vec3d5.zCoord * vec3d4.zCoord);
                if (f8 >= -0.15f && (iblockstate = this.world.getBlockState(blockpos = new BlockPos(this.posX, this.getEntityBoundingBox().maxY, this.posZ))).getCollisionBoundingBox(this.world, blockpos) == null && (iblockstate2 = this.world.getBlockState(blockpos = blockpos.up())).getCollisionBoundingBox(this.world, blockpos) == null) {
                    float f15;
                    float f9 = 7.0f;
                    float f10 = 1.2f;
                    if (this.isPotionActive(MobEffects.JUMP_BOOST)) {
                        f10 += (float)(this.getActivePotionEffect(MobEffects.JUMP_BOOST).getAmplifier() + 1) * 0.75f;
                    }
                    float f11 = Math.max(f * 7.0f, 1.0f / f7);
                    Vec3d vec3d6 = vec3d2.add(vec3d4.scale(f11));
                    float f12 = this.width;
                    float f13 = this.height;
                    AxisAlignedBB axisalignedbb = new AxisAlignedBB(vec3d, vec3d6.addVector(0.0, f13, 0.0)).expand(f12, 0.0, f12);
                    Vec3d lvt_19_1_ = vec3d.addVector(0.0, 0.51f, 0.0);
                    vec3d6 = vec3d6.addVector(0.0, 0.51f, 0.0);
                    Vec3d vec3d7 = vec3d4.crossProduct(new Vec3d(0.0, 1.0, 0.0));
                    Vec3d vec3d8 = vec3d7.scale(f12 * 0.5f);
                    Vec3d vec3d9 = lvt_19_1_.subtract(vec3d8);
                    Vec3d vec3d10 = vec3d6.subtract(vec3d8);
                    Vec3d vec3d11 = lvt_19_1_.add(vec3d8);
                    Vec3d vec3d12 = vec3d6.add(vec3d8);
                    List<AxisAlignedBB> list = this.world.getCollisionBoxes(this, axisalignedbb);
                    list.isEmpty();
                    float f14 = Float.MIN_VALUE;
                    for (AxisAlignedBB axisalignedbb2 : list) {
                        if (!axisalignedbb2.intersects(vec3d9, vec3d10) && !axisalignedbb2.intersects(vec3d11, vec3d12)) continue;
                        f14 = (float)axisalignedbb2.maxY;
                        Vec3d vec3d13 = axisalignedbb2.getCenter();
                        BlockPos blockpos2 = new BlockPos(vec3d13);
                        int i = 1;
                        while ((float)i < f10) {
                            IBlockState iblockstate4;
                            BlockPos blockpos3 = blockpos2.up(i);
                            IBlockState iblockstate3 = this.world.getBlockState(blockpos3);
                            AxisAlignedBB axisalignedbb3 = iblockstate3.getCollisionBoundingBox(this.world, blockpos3);
                            if (axisalignedbb3 != null && (double)(f14 = (float)axisalignedbb3.maxY + (float)blockpos3.getY()) - this.getEntityBoundingBox().minY > (double)f10) {
                                return;
                            }
                            if (i > 1 && (iblockstate4 = this.world.getBlockState(blockpos = blockpos.up())).getCollisionBoundingBox(this.world, blockpos) != null) {
                                return;
                            }
                            ++i;
                        }
                    ...
                    if (f14 != Float.MIN_VALUE && (f15 = (float)((double)f14 - this.getEntityBoundingBox().minY)) > 0.5f && f15 <= f10) {
                        this.autoJumpTime = 1;
                    }
                }
            }
